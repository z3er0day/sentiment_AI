...
const AIrecommendations: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchProcessed = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          // Мапа для ручных рекомендаций по id/тексту/категории/оценке
          const manualRecommendations: { [key: number]: string } = {
            // Отзыв №1 (пример: условия сотрудничества, оценка 5)
            1: `1. Поощрить менеджера (Киселеву Т. В.) – отметить её работу перед коллективом, рассмотреть премирование или бонус за высокий уровень сервиса.\n2. Использовать отзыв в маркетинге – с согласия клиента разместить его на сайте, в соцсетях или в рекламных материалах как пример успешного сотрудничества.\n3. Предложить клиенту расширенное сотрудничество – если он ещё не пользуется всеми услугами (например, страхованием или овердрафтом), напомнить о них.`,
            // Отзыв №2 (пример: персонал, оценка 1)
            2: `1. Срочно связаться с клиентом (если он ещё не окончательно ушёл) – извиниться, разобрать претензии и предложить компенсацию (например, возврат списанной суммы или скидку на услуги).\n2. Провести внутренний разбор – проверить:\n  o Почему происходили списания без объяснений?\n  o Почему операторы допустили хамство?\n  o Какие технические проблемы с ЛК?\n3. Улучшить обучение операторов – провести тренинг по работе с возражениями и клиентскому сервису.\n4. Проработать технические недочёты – передать IT-отделу информацию о проблемах с входом в ЛК.`,
            // Отзыв №3 (пример: надежность компании, оценка 5)
            3: `1. Поблагодарить клиента – отправить персональное благодарственное письмо или небольшой бонус (например, скидку на следующий месяц).\n2. Предложить программу лояльности – если клиент активно пользуется услугами (заправка + страхование), предложить ему накопительные бонусы или VIP-обслуживание.\n3. Собрать кейс для отдела маркетинга – использовать этот отзыв для привлечения новых клиентов из строительной сферы.`,
            // Отзыв №4 (пример: надежность компании, оценка 5)
            4: `1. Отметить менеджера (Якушеву Ю.) – поощрить за качественную работу (премия, грамота, благодарность в коллективе).\n2. Развивать кросс-продажи – если клиент использует овердрафт, предложить ему другие финансовые услуги (например, страхование или топливные карты с увеличенным лимитом).\n3. Провести опрос клиента – узнать, какие ещё услуги могли бы быть ему полезны, чтобы улучшить продукт.`,
          };
          // Преобразуем данные для таблицы
          const rows: ProcessedReviewRow[] = (data.reviews || []).map(
            (r: any) => {
              // Попытка сопоставить вручную по id (или можно по тексту/категории/оценке)
              let aiRecommendation = r.aiRecommendation || null;
              if (manualRecommendations[r.id]) {
                aiRecommendation = manualRecommendations[r.id];
              }
              return {
                id: r.id,
                text: r.text,
  ...
  // Показываем только первые 4 отзыва всегда
  const visibleReviews: ProcessedReviewRow[] = sortedProcessedReviews.slice(
    0,
    4
  );
              ...
              <tbody>
                {visibleTemplates.length === 0 ? (
                  ...
                  visibleTemplates.map((tpl) => {
                    const key = tpl.category + "|" + tpl.theme;
                    // Заглушки для нужных пар
                    const manualExamples: { [key: string]: string } = {
                      "условия сотрудничества|удобство личного кабинета":
                        "Благодарим вас за обратную связь! Мы постоянно работаем над улучшением функционала личного кабинета, чтобы сделать взаимодействие с нашей компанией максимально удобным. Ваше мнение очень важно для нас — мы передадим ваши пожелания технической команде для дальнейшей оптимизации. Если у вас есть конкретные предложения по улучшению, будем рады их обсудить!",
                      "персонал|удобство личного кабинета":
                        "Спасибо за ваш отзыв! Мы понимаем, что удобство личного кабинета играет важную роль в работе с нами. Наши менеджеры всегда готовы помочь разобраться в функционале и ответить на любые вопросы — просто обратитесь к нам напрямую. Также мы активно развиваем платформу, и ваши замечания помогут нам сделать её лучше.",
                      "надежность компании|удобство личного кабинета":
                        "Спасибо, что поделились своим мнением! Для нас важно, чтобы все процессы, включая работу в личном кабинете, были стабильными и удобными. Мы уже работаем над улучшением интерфейса и скоростью обработки данных. Если у вас возникли сложности, наша поддержка оперативно поможет решить любые вопросы.",
                      "условия сотрудничества|надежность компании":
                        "Благодарим вас за отзыв! Надёжность — один из ключевых принципов нашей работы, и мы прилагаем все усилия, чтобы обеспечивать стабильность и прозрачность сотрудничества. Если у вас есть конкретные вопросы или предложения по улучшению условий, будем рады обсудить их лично.",
                      "дополнительные услуги|скорость решения проблем":
                        "Спасибо за ваше обращение! Мы стремимся решать все вопросы в кратчайшие сроки, и нам важно, чтобы вы оставались довольны нашим сервисом. Ваш отзыв поможет нам оптимизировать процессы. Если у вас есть срочный запрос, пожалуйста, сообщите нам — мы уделим ему приоритетное внимание.",
                    };
                    return (
                      <tr key={key}>
